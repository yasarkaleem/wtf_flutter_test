...
class CallBloc extends Bloc<CallEvent, CallBlocState> {
  ...
  CallBloc({
    required CallRepository callRepo,
    ...
    _subscribeToCallRepo();
  }

  void _subscribeToCallRepo() {
    _subs.add(_callRepo.onJoinStream.listen((room) => add(_CallJoined(room))));
    _subs.add(_callRepo.peerUpdateStream.listen(
      (e) => add(_CallPeerUpdate(e.peer, e.update)),
    ));
    _subs.add(_callRepo.trackUpdateStream.listen(
      (e) => add(_CallTrackUpdate(e.track, e.update, e.peer)),
    ));
    _subs.add(_callRepo.errorStream.listen(
      (e) => add(_CallError(e.message ?? 'Unknown error')),
    ));
    _subs.add(_callRepo.reconnectStream.listen(
  ...
  Future<void> _onJoin(CallJoin event, Emitter<CallBlocState> emit) async {
    ...
    try {
      ...
      // Build SDK
      await _callRepo.build();

      // Get token
      final token = await _callRepo.getAuthToken(
        roomId: event.roomId,
        userId: user.id,
        role: event.role,
      );
      ...
      await _callRepo.join(token: token, userName: user.name);
    } catch (e) {
  ...
  Future<void> _onLeave(CallLeave event, Emitter<CallBlocState> emit) async {
    ...
    await _callRepo.leave();
    await _callRepo.destroy();
